...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			...
			-- Calculate end date as one day before the next start day
		...
		SELECT
			...
			END sls_sales,
			sls_quantity,
			-- Devide price if original value is invalid
		...
		SELECT 
			...
			CASE WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid, 4, LEN(cid))
				ELSE cid
			END AS cid,
			-- Set future birthdaates to NULL
			CASE WHEN bdate > GETDATE() THEN NULL
				ELSE bdate
			END AS bdate,
			-- Normalize gender values anf handle unknown cases
		...
		SELECT
			REPLACE(cid, '-', ''),
			-- Normalizer and Handle missing or blank country codes
	...
	BEGIN CATCH
		PRINT('======================================');
		PRINT('ERROR OCCURED DURING LOADING SILVERLAYER');
